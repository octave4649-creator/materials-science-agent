...
本手册面向已报名或计划报名 GOAI 【前沿探索  丨  AI for Research】赛道的参赛团队 ，帮助选手了解赛道方向、 参赛路径、各阶段提交要求、技术要求、 评审标准和答疑渠道。
...
这意味着真正稀缺的 ，不再是会用AI跑实验的人 ， 而是**能把科学直觉转化为可计算探索环境****的人**——能重新定义「AI 时代的科学应该长什么样」 的人 ，这正是 Track 3 要找的。
...
l 具备 AI/ML 技术能力 ， 同时对某个科学领域（生物学、 化学、材料、 天文、物理等） 有真实认知的跨学科团队

l 博士生、 博士后、 年轻 PI及其团队 ，拥有真实未解科学问题并希望探索 AI 解法的研究者


l 有计算生物学、 计算化学、 计算材料等方向背景的高校实验室、科研团队


l 对 Agent RL、 自进化系统、科学发现 benchmark 设计有研究兴趣的AI技术团队


l 希望通过赛事建立 AI4Science 领域影响力和社区连接的开源团队
...
| 赛道亚军 | 2 名 | 各 15 万 RMB |
| 赛道季军 | 2 名 | 各 5 万 RMB |
| 科学家直通奖 | 1-2 名 | 各 2 万 RMB<br>由科学家评委直接提名 ，跨两个通道 ，独立于冠亚季军之外。 另附联合研究机会或算力资源对接。 |


| 奖项 | 数量 | 说明 |
| --- | --- | --- |
| 范式创造奖 | 0-1 名 | 2  万 RMB<br>如有参赛者设计出有说服力的动态科学评测框架，由全体评委联合提名 ， 单独颁发。 |
| 优胜奖 | 入围复赛团队 | 500RMB  纪念礼品包/队伍 |
| 参与奖 | 提交有效参赛方案团队 | 大赛纪念  T  恤/队伍 |
...
| 资源类型 | 是否提供 | 支持对象 | 说明 |
| --- | --- | --- | --- |
| 科学数据集 | 是 | 全体参赛者 | 算法类赛题的公开数据集与 Benchmark 于7月 15 日通过官网发布 ， 以官网最新版本为 准。 |
| 技术文档 / 教程 | 是 | 全体参赛者 | 含赛题说明、 评测框架、环境骨架参考、样例库与提交流程。 具体安排组委会将通过社群、邮件等方式另行公布。 |
| 科学家评委答疑 | 是 | 复赛团队 | 各方向安排在线答疑； 开放探索赛题参赛者可申请问题预审（可选） 。具体安排组委会将通过社群、 邮件等方式另行公布。 |
...
| 初赛 | 7.16-<br>8.16 | 8. 16 初赛作品提交截止 | l 探索赛： 问题定义文档（4 页）<br>l 算法赛：初步方案与阶段性实验结果 |
| 初赛评审 | 8.17-<br>8.24 | 8.24 公布复赛入围队伍名单，Top 4 0 队伍进入复赛 | 无新增提交 ， 除非组委会另行要求 |
| 复赛 | 8.25-9.3 | 9.3 复赛作品提交截止 | l 探索赛： 完整三件套<br>l 算法赛： 最终代码 + 完整实验结果 |
| 复赛评审 | 9.4-9.10 | 9. 10 公布决赛入围名单，Top 2 0 队伍进入决赛 ，受邀参加线下决赛答辩 ，角逐最终奖项 | 无新增提交 ， 除非组委会另行要求 |
| 决赛 | 9.22 | 线下决赛答辩与展示 | l 探索赛： 决赛路演 PPT、现场 Demo、项目一页纸、 答辩材料<br>l 算法赛： 海报 + 展示材料 |
...
| 科学背景 | 围绕细胞状态、扰动响应与调控关系 ，构建可计算的预测与推理任务 ，让AI从拟合数据走向建模与推理细胞行为。 |
| --- | --- |
...
| 评测标准 | 在  MISATO （Nature  Computational Science, 2024） 数据集上统一评测 ， 并与  NeuralMD（Nature Communications, 2025） 等  baseline  比较。 |
...
| --- | --- |
| 赛题要求 | 基于大规模材料科学文献库 ，构建能够自主阅读、推理并产出可验证科学发现的智能体系统。 要求根据文献调研结果 ，产出具有可证伪性的科学发现一包括材料_性质关联、 隐藏知识连接、 新材料设计假设等。 所有队伍须先完成 「文献调研 Agent」 基础任务 （自主检索筛选 、 知识抽取 、 Research Gap 识别、 结构化调研报告） ， 再从构效关系发现 、模拟方法 |


|  | 创新、 合成路线与工艺设计三条路线中任选其一深入。 |
| --- | --- |
| 评测标准 | 基础任务(文献调研那部分)应该考察:Research Gap 的识别准确率与新颖性口文献溯源的完整性与可信度调研报告的结构化程度与可读性等进阶任务的考察标准依赖于具体路线 ，详见下文。 |
| 数据与资源 | 推荐使用 Sciverse 科学智能数据库（2500 万+篇经深度解析的开放获取文献 ，支持语义检索与全文定位） 与开源文档解析引擎 MinerU； 鼓励通过 Sciverse API 的  MCP/Skill 接入 ， 其调用记录天然构成可审计的证据链。 开放数据集可经 Hugging Face（opendata lab/Sci-Base） 获取 ， 具体以官网赛题文档为准。 |
...
| 阶段 | 是否必交 | 提交材料 | 内容说明 |
| --- | --- | --- | --- |
| 初赛 | 是 | 方案说明文档、技术路线概述 | 说明所选方向、科学问题理解、技术方案与预期方法路线； 可附初步实验或可行性验证。 |
| 复赛 | 是 | 可运行代码仓库、 实验结果报告、科学意义阐释、依赖披露 | 提交完整代码（含 README、环境配置、运行说明） 、完整实验结果 ， 以及方法的科学意义说明与依赖/授权披露。 |
| 决赛 | 是 | 海报 / 路演材料、 现场展 | 聚焦方法、核心结果与科学意义； 现场展示与 |
...
| 阶段 | 是否必交 | 提交材料 | 内容说明 |
| --- | --- | --- | --- |
| 初赛 | 是 | 问题定义文档   (≤ 4 页） | 说明看见了什么问题、 为什么AI 可以介入、 如何定义探索环境、什么算发现信号、 最小参照系是什么。 |
| 复赛 | 是 | 最小可运行探索环境、 探索日志、 参照系设计 | 提交可运行环境、 至少一次完整运行日志、 随机探索或平凡解等参照系 ， 以及 README 与复现说明。 |
| 决赛 | 是 | 探索报告、 答辩材料、 最终代码仓库 | 说明探索过程中的正结果、 负结果、 异常或问题修正究竟意味着什么； 负结果被明确允许。 |
...
| 是否允许使用商业API | 允许 ，但必须披露调用环节 、 费用假设、 权限范围、 可替代性与对可复现性的影响。 |
...
| 开源贡献 | 5% | 代码、模型或数据处理管线的开源质量与可复用性 ，及可被社区接续使用与复现的程度。 |
| --- | --- | --- |
...
| 可检查性与可延续性 | 20% | 是否有最小参照系、运行日志、复现说明和后续研究路径； 能否沉淀为社区可接续的问题包或环境包。 |
...
| 资源类型 | 是否提供 | 支持对象 | 说明 |
| --- | --- | --- | --- |
| 科学数据集 | 是 | 全体参赛者 | 算法类赛题的公开数据集与 Benchmark 于 7 月15  日通过官网发布 ， 以官网最新版本为准。 |
| 算力资源 | 待确认 | 入围团队 | 用于算法方案运行验证、模型训练与探索环境运行。 |


| LLM API / 模型额度 | 部分提供 | 参赛团队 | 部分赛题（如自动出题-解题类）报销指定   LLMAPI 调用与 coding agent 费用； 其余待确认。 |
| --- | --- | --- | --- |
| 自动化评审平台 | 规划中 | 算法类赛题 | 算法赛的代码提交、 自动运行与结构化评<br>分 ，拟对接成熟赛事平台 ，具体以官网通知为准。 |
| 技术文档 / 教程 | 是 | 全体参赛者 | 含赛题说明、 评测框架、环境骨架参考、样例库与提交流程。 |
| 科学家评委答疑 | 是 | 全体参赛者 | 各方向安排在线答疑； 开放探索赛题参赛者可申请问题预审（可选）。 |
| 导师辅导 | 待确认 | 入围团队 | 复赛辅导期 ，覆盖方案评审、科学背景对齐与开源规范。 |


资源使用说明：涉及商业API、 闭源模型、私有数据或受限资源的使用 ， 需在提交材料中说明用途、成本、权限范围、 替代方案与合规限制。 具体申请方式以官网公告和官方表单为准。


<u>**09**</u><u>  </u><u>**赛题**</u><u> </u><u>**FAQ**</u><u>                                                                                                                          </u>
...
可以基于现有方法继续开发 ，但需说明原方法来源、 团队的新贡献点 ， 以及相对已有 baseline的改进。
...
算法类赛题的公开数据集与 Benchmark 于7 月 中下旬通过官网发布。 请以官网发布的最新版本为准 ，本资料仅作方向说明。
...
*本资料为当前规划版本，* *赛程、* *奖项、* *资源、* *评委与提**交方式可能根据赛事安排调整，* *最终以* *GOAI* *世界人工*

*智能开源大赛组委会正式发布为准。*


GOAI世界人工智能开源大赛 · 赛道三：前沿探索AI for Research
...
组委会提供两个输入文件： train_val 与 test。 train_val 含样本 metadata、 条件信息 、测量上下文与蛋白质组标签 ，用于训练与内部验证； 参赛队可自行在 train_val 内划分训练/验证 ，或采用交叉验证做模型选择。 test 为独立评测集 ， 发布样本 ID、 生物条件与测量上下文 ，蛋白质组真值标签保留、 由组委会离线评分。

每个样本对应一个固定蛋白顺序的 log2 蛋白丰度向量。 输入特征包括生物条件（菌株、 化合物、 培养基、 温度、 时间） 与测量上下文（数据来源、仪器、 板号等采集元数据） ；测量上下文用于吸收测量过程引入的变异 ，而泛化类型仍由菌株与化合物实体的可见性定义。


| 数据类型 | 建议公开字段 | 用途 |
| --- | --- | --- |
| 扰动蛋白质组数据 | 菌株、化合物/处理、培养基、温度、 时间点、重复、蛋白丰度、批次 | 主预测目标；评估模型对细胞<br>响应状态的刻画能力 |
| 公共外部数据 | 公开基因组、转录组、蛋白质组、代谢通路、基因注释与文献数据库 | 开放知识榜可用；封闭数据榜<br>仅用于赛后解释或非排名分析 |
...
| 评价场景 | 划分方式 | 考察能力 |
| --- | --- | --- |
| S1 | 已见菌株 + 未见化合物 | 对新扰动的外推能力 |
| S2 | 未见菌株 + 已见化合物 | 对新遗传背景的外推能力 |
| S3 | 未见菌株 + 未见化合物 | 对双重未知组合的泛化能力 |
...
| --- | --- | --- |
| 绝对保真度（全蛋白质组真实性） | 20% | 适用全部划分。逐样本 corr / R²（每个样本的预测向量对真值向量）<br>与逐蛋白 corr / R²（每个蛋白跨样本）： PCC 刻画蛋白谱形状 ， R² 刻画幅度与校准（可为负）。样本轴与蛋白轴分别聚合 ，避免高丰度易预测蛋白掩盖困难样本。 |


| 评分模块 | 权重 | 计算方式、适用划分与取向 |
| --- | --- | --- |
| 匹配对照原始 FC（扰动效应预测） | 25% | 所有 OOD 划分的核心指标。Δ_pred = ŷ_treat − y_control ，Δ_true  = y_treat − y_control ，计算 PCC(Δ_pred, Δ_true)。对照须匹配来源、 菌株、培养基、温度、 时间、仪器、板号 ，并按官方规则匹配到DMSO 或 Water。评估模型能否捕捉药物/扰动引起的真实蛋白质组变化。 |
| 上下文均值残差（新化合物的特异响应） | 20% | 适用 test_chem_only（对应 S1）等新化合物划分。 μ_ctx = 同上下文下训练药物的 Δ_true 均值；计算 PCC(Δ_pred − μ_ctx, Δ_true − μ_ctx)。剔除同一菌株/培养基/温度/时间/冻结批次下的共享药物响应分量 ，聚焦留出化合物的特异作用机制方向。 |
| 药物均值残差（新菌株/上下文的背景调制） | 20% | 适用 test_strain_only（对应 S2）等新菌株划分。 μ_drug = 同药物下训练上下文的 Δ_true 均值；计算 PCC(Δ_pred − μ_drug, Δ_true − μ_drug)。剔除同一药物共享的平均作用机制谱 ，评估模型能否预测未见菌株/上下文如何调制该响应。 |
| 双重未知 / 时间外推 | 10% | 适用 test_both（对应 S3） 与 test_time。test_both 以原始 FC + 绝对保真度为主；test_time 以绝对保真度 + 原始 FC 为主。组合外推缺乏稳定的训练实体均值 ，不强制以残差为唯一主指标 ，重点在完整蛋白质组与扰动方向是否可靠。 |
| 高效应蛋白与 DEP 检出 | 5% | 全部划分的补充指标。对 |Δ_true| > 1 的蛋白 ，计算方向准确率、 高效应 PCC、 Recall@K、 F1 或 AUPRC。 Recall 不单用 ，须与<br>precision、 F1 或 AUPRC 组合 ，避免「全报阳性」刷高召回。 |
...
| 阶段 | 提交物 | 要求 |
| --- | --- | --- |
| 主榜阶段 | prediction.csv | 按官方模板提交所有样本的响应预测值 ，不得修改样本 ID |
| 复现审核 | 代码包或容器、说明文档 | 说明训练流程、外部资源、 随机种子与运行方式；开放知识榜须列出全部外部数据来源 |

**科学意义：**

为菌株定向改造与优化提供数据支撑 ，是「AI 预测 → 湿实验验证」 闭环的关键一环。
...
•   **数据来源 ：**MISATO 起始于 PDBbind 的约 2 万个实验解析的蛋白–配体复合物 ，经半经验量子力学系统性精修 ， 并在水环境下进行分子动力学模拟 ，是目前规模最大的公开小分

子–蛋白质结合动力学数据集之一。

•   **规模划分：**** **13,066 个复合物轨迹用于训练 ， 1,357 个复合物轨迹用于验证与测试； 测试集为全新复合物 ，测试轨迹真值不发布 ， 由组委会保留用于评分。
...
| 评价场景 | 划分方式 | 考察能力 |
| --- | --- | --- |
| T1 | 短程预测（predict next k frames） | 局部动力学 |
| T2 | 长观测预测（train on first 80%, test on last 20%） | 较充分观测下的未来预测 |
| T3 | 少观测预测（train on first 20%, test on last 80%） | 有限观测下的长期预测 |


**评分方法：**


单个评测集得分按四类指标加权 ，各指标均相对官方基线归一化为 0–100 分： Score_t = 40% ×Geo_t + 25%×Phys_t + 25%×Dyn_t + 10%×Stab_t。 最终主榜以主评测为核心、 两档泛**化挑战为辅： **Final = 50%×Score_T1 + 30%×Score_T2 + 20%×Score_T3。 主榜排名以最终测试集成绩为准 ，验证集成绩只作阶段反馈。


| 评分模块 | 评分对象 | 评分方式与取向 |
| --- | --- | --- |
| 几何重建 Geo | 预测轨迹相对真值的坐标误差（配体重原子 RMSD、 口袋原子偏差 ，按帧并跨时程汇总） | 相对基线重建误差归一化；奖励逐帧几何贴近真值、结合位姿不漂移 |
| 物理合法性Phys | 键长/键角、原子间碰撞、 立体化学与能量合理性 | 以合法帧比例与违例程度评分（对应基线的 validity） ；惩罚穿模、 断键、 非物理构象 |
| 动力学观测量Dy n | 关键接触/氢键距离分布、 RMSF 涨落谱、回转半径、沿集体变量的构象/自由能分布与时间相关性 | 以分布距离（如 JS / Wasserstein） 与相关性评分； 奖励还原真实涨落与动力学特征 ，而非只贴近均值构象 |
| 长时程稳定性Stab | 短 / 中 / 长时程多时间窗下的误差增长与轨迹稳定性 | 按 rollout 漂移与长时程稳定性评分；奖励长时程不发散、不塌缩 |

**评分口径要点：**

•   **基线：**考虑到算力需求， 先计算 semi-flexible setting， 即蛋白结构在轨迹中不动。以 NeuralMD（Nature Communications, 2025） 为主基线——其以物理约束的多粒度群对称框架（BindingNet + 增广神经微分方程求解器） 学习牛顿力学下的结合轨迹 ，并以「重建误差」 与「合法性」 为核心评价维度； 本赛沿用并扩展该口径 ，增加动力学分布与长时程稳定性。
...
| 复现审核 | 代码包或容器、说明文档 | 说明训练流程、外部资源、 随机种子与运行方式； 如使用外部数据或预训练模型 ，须列出全部来源 |
| 决赛应用题 | 设计报告 | 在组委会指定的一组靶点–配体上 ，用所学动力学模型完成一个下游任务（如结合稳定性排序或口袋柔性 / 动力学代理量估计） ，并与保留的高精度 MD 或实验证据比对 |
...
本方向面向材料科学领域 ，核心考察参赛者能否构建一个从文献出发、产出可验证科学发现的智能体系统。 要求根据文献调研结果 ，产出具有可证伪性的科学发现一包括材料_性质关联、隐藏知识连接、 新材料设计假设等。
...
**推荐工具： **Sciverse 科学智能数据库 （2500 万 + 篇论文 ， 支持语义检索与全文定位） 、 MinerU（开源文档解析引擎 ，支持 PDF 到结构化内容的深度解析） ；鼓励使用 Sciverse API的 MCP/Skill 接入方式 ，其调用记录天然构成可审计的证据链。
...
| 资源 | 内容 | 获取方式 |
| --- | --- | --- |
| Sci-Base | 2500 万+篇论文、 6000 亿+ tokens ，覆盖含材料科学在内的 10 个学科 | Hugging Face：<br>opendata lab/Sci-Base |
| Sciverse API | 4.66 亿条学术元数据、2800 万+全文证据片段 ，语义检索 | sciverse.opendatalab.com注册 |
| Miner U | 开源文档解析引擎（PDF→结构化内容） | mineru. net |
...
•   **进阶路线（50%****）**** ****：**** **路线 A 综合评分 ，重点考察搜索方法与 LLM 的融合深度 ， 以及物理机制解释的说服力、对已有理论的推广甚至颠覆程度；路线 B 看公认 benchmark 表现或细分领域改进幅度 ，考察创新性与实用性；路线 C 看合成路线可行性、优化方案有效性、验证手段充分性。
...
**设计理念：**基本任务保证门槛（所有参赛者至少有文献调研 Agent 的产出 ，避免无从下手） ；三条路线覆盖知识发现、 计算方法创新与工程应用三类能力 ，不同背景的参赛者都能找到发力点； 每条路线都配有明确评估标准与可用数据源 ，避免空中楼阁； 加分项鼓励实验闭环 ，推动「预测→验证」 的科研闭环。
...
| 评审维度 | 建议权重 | 说明 |
| --- | --- | --- |
| 技术性能 | 45% | 方向内量化成绩经归一化后的表现——虚拟细胞取最终主榜的六模块加权总分 ，小分子取三场景 T1/T2/T3 加权的最终主榜得分（相对<br>NeuralMD 基线归一化） ，材料取「基本任务 + 进阶路线」综合分。各方向均以相对基线 / 相对参照的归一化分进入 ， 以保证跨方向可比。 |
| 科学意义 | 30% | 结果对该科学领域是否有真实意义 ，是否超出已有工作的边界。 |
| 方法创新性 | 20% | 方法是否有新颖的技术设计 ，而非对已有方法的简单集成。 |
...
*注：* *本资料为当前规划版本，* *赛程、* *数据集、* *评测细则、* *资源与提交方式**可能根据赛事安排调整，* *最终以**GOAI* *世界人工智能开源大赛组委会正式发布为准。*
