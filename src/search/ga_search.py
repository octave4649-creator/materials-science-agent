...
# 常见热电掺杂元素池（文献高频掺杂位点）
DOPANT_POOL = ["Ti", "Bi", "Sb", "Na", "Cu", "Ag", "In", "Mn", "Cd", "Se", "La"]
...
class LLMRoles:
    ...
    def evaluate(self, cands: list[Candidate]) -> dict[str, dict[str, float] | str] | None:
        ...
        user = "\n".join(
            f"- {c.formula}（{c.host} 掺 {c.dopant} {c.concentration}%，"
            f"理由：{c.rationale[:80]}）" for c in cands
        )
        try:
...
def ga_search(
    *,
    ...
    if llm_on:
        ...
        if seeds:
            ...
            log.add(generation=0, action="seed", n_candidates=len(pop),
                    llm_role="generator", detail=f"LLM 生成 {len(pop)} 个种子候选")
    if not pop:
    ...
    for gen in range(1, generations + 1):
        log.add(generation=gen, action="evaluate", n_candidates=len(pop),
                llm_role="evaluator", detail="批量评估")
        # 评估（LLM 或规则）
        if llm_on:
            ...
            if results:
                for c in pop:
                    ...
                    if r and isinstance(r, dict):
                        ...
                        c.verdict = (
                            r.get("verdict", "keep")
                            if r.get("verdict") in ("keep", "drop") else "keep"
                        )
                        if r.get("reason"):
        ...
        if neg:
            ...
            for c in pop:
                if c.host in neg:
                    c.verdict = "drop"
            if fb_dropped:
                log.add(generation=gen, action="prune_feedback",
            ...
            if drops:
                ...
                for c in pop:
                    if c.formula in drop_set:
                        c.verdict = "drop"
                log.add(generation=gen, action="prune", n_candidates=len(drop_set),
                        llm_role="pruner", detail=f"LLM 淘汰 {len(drop_set)} 个候选")
        ...
        log.add(generation=gen, action="select", n_candidates=len(elite),
                detail=f"保留精英 {len(elite)} 个，Top 均分 "
                       f"{best[0].score_avg() if best else 0:.2f}")
        ...
        if gen < generations:
            ...
            while len(elite) + len(children) < pop_size:
                ...
                if rng.random() < 0.5:
                    ...
                    child = Candidate(
                        host=base.host, dopant=base.dopant, concentration=conc,
                        formula=_nominal_formula(base.host, base.dopant, conc),
                        rationale=f"变异浓度：{base.formula} → {conc}%", source="ga_mutation",
                    )
                else:
                    dopant = rng.choice(DOPANT_POOL)
                    child = Candidate(
                        host=base.host, dopant=dopant, concentration=base.concentration,
                        formula=_nominal_formula(base.host, dopant, base.concentration),
                        rationale=f"变异掺杂元素：{base.formula} → {dopant}", source="ga_mutation",
                    )
                children.append(child)
            log.add(generation=gen, action="crossover_mutate", n_candidates=len(children),
    ...
    log.add(generation=generations, action="done", n_candidates=len(best),
